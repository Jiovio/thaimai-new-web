...
$checkvaliduser = $db->prepare("SELECT * FROM antenatalvisit av join ecregister ec on ec.picmeNo=av.picmeno WHERE ec.BlockId='".$data->block."' AND ec.PhcId='".$data->phc."' AND ec.HscId='".$data->hsc."' AND av.picmeno='".$data->picmeno."' AND av.status=1");
...
if ($checknum >0) {
     while ( $row = $checkvaliduser->fetch(PDO::FETCH_ASSOC)) 
     {
        ...
        $anarray[] = array( 
            ...
            "lT1" =>$row['lT1'],
            "usgFHRT1" =>$row['usgFHRT1'],
            "usgFPT1" =>$row['usgFPT1'],
            "usgFMT1" =>$row['usgFMT1'],
            "lT2" =>$row['lT2'],
            "usgFHRT2" =>$row['usgFHRT2'],
            "usgFPT2" =>$row['usgFPT2'],
            "usgFMT2" =>$row['usgFMT2'],
            "lT3" =>$row['lT3'],
            "usgFHRT3" =>$row['usgFHRT3'],
            "usgFPT3" =>$row['usgFPT3'],
            "usgFMT3" =>$row['usgFMT3'],
