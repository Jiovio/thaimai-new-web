...
if (!empty($data->picmeno)) {
    ...
    // $form->usgreport = $data->usgreport;
    // if ($form->usgreport !='') {
    //     $filename = $form->usgreport;
    ...
    //         $filename = $image;
    //         file_put_contents("../usgDocument/" . $filename,$binary);
    ...
    $image = $data->usgreport;
     if ($image != '') {
            $filename = $image . ".jpg";
            move_uploaded_file($_FILES[$image]["tmp_name"], "../usgDocument/" . $_FILES[$image]["name"]);
          // $file = fopen('usgDocument/' . $filename, 'wb');
            //$binary = base64_encode($image);
          // header('Content-Type: bitmap; charset=utf-8');
            // $file = fopen('usgDocument/' . $filename, 'wb');
            // fwrite($file, $binary);
            // fclose($file);
            $form->usgreport = '../usgDocument/' . $filename;
        } else {
            $form->usgreport = '';
        }
    //$form->usgreport = $data->usgreport;
    ...
    echo json_encode(array("success" => "false", "error" => "true","message" => "Please Check the Mother Aadhaar ID. Antenatal Visit not Created."));  
}
}
