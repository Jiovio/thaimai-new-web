...
if (!empty($data->picmeNo)) {
   ...
   // if($ddate1 >0) {
    ...
    if($data->motherCovidVac1Type == '1') {
      $form->Fcov2DueDate = date('Y-m-d', strtotime($data->motherCovidVac1Date. '+ 30 days' ));
    } else if($data->motherCovidVac1Type == '2'){
      $form->Fcov2DueDate = date('Y-m-d', strtotime($data->motherCovidVac1Date. '+ 90 days' ));
    ...
    if($data->motherCovidVac2Type == '1') {
      $form->FBcovDueDate = date('Y-m-d', strtotime($data->motherCovidVac2Date. '+ 180 days' ));
    } else if($data->motherCovidVac2Type == '2'){
      $form->FBcovDueDate = date('Y-m-d', strtotime($data->motherCovidVac2Date. '+ 180 days' ));
    ...
    if($data->$motherCovidVac1Type == '1' || $data->$motherCovidVac1Type == '2' ) {
      $form->$NextDoseName1 = $data->$motherCovidVac1Type;
    }else if($data->$motherCovidVac2Type == '1' || $data->$motherCovidVac2Type == '2'){
      $form->$NextDoseName1 = $data->$motherCovidVac2Type;
      
    } else if($data->$motherCovidVacBoosterType == '1' || $data->$motherCovidVacBoosterType == '2'){
      $form->$NextDoseName1 = $data->$motherCovidVacBoosterType;
    ...
    if ($mvalidchecknum >0) {
        $inserid = $form->editimmunization();
        // set response code - 201 created
        http_response_code(200);

        echo json_encode(array("success" => "true", "error" => "false", "message" => "Immunization Updated Successfully."));
    } else {
...
// tell the user data is incomplete
    ...
    http_response_code(400);
    // tell the user
    echo json_encode(array("success" => "false", "error" => "true", "message" => "Unable to Update Immunization. "));
}
?>