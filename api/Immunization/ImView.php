...
$checkvaliduser = $db->prepare("SELECT * from immunization im JOIN ecregister ec on im.picmeNo=ec.picmeno WHERE ec.BlockId='".$data->block."' AND ec.PhcId='".$data->phc."' AND ec.HscId='".$data->hsc."' AND im.picmeNo='".$data->picmeNo."' AND im.status=1 ORDER BY ec.motheraadhaarname ASC");
