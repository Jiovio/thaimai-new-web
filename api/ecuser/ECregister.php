...
if (!empty($data->ecfrno)) {
    ...
    if ($mvalidchecknum == 0) {
        $inserid = $form->createecregister();
        // set response code - 201 created
        http_response_code(200);
        // tell the user
        echo json_encode(array("success" => "true", "register_id" => $inserid,"message" => "User created."));
    } else {
       
        http_response_code(400);
        // tell the user
        echo json_encode(array("success" => "false", "error" => "true","message" => "Ecno already Entered."));
