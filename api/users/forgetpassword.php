...
if (!empty($data->mobile)) {
    ...
    $mvaliduser1 = $db->prepare("SELECT * FROM users WHERE mobile='".$data->mobile."' AND status=1 ORDER BY id ASC");
...
if(!empty($data->encpassword) && !empty($data->password)) {
    ...
    $mvalidusers = $db->prepare("SELECT * FROM users WHERE mobile='".$data->mobile."' AND status=1 ORDER BY id ASC");
