...
//if (!empty($data->picmeno) && !empty($data->motheraadhaarid)) {

//$checkvaliduser = $db->prepare("SELECT * FROM antenatalvisit WHERE status=1 ORDER BY id ASC");
$checkvaliduser = $db->prepare("SELECT * FROM antenatalvisit av JOIN ecregister ec on ec.picmeNo=av.picmeno JOIN users u on av.createdBy=u.id WHERE avdueDate>=DATE_FORMAT(NOW() ,'%Y-%m-01') AND ec.BlockId='".$data->block."' AND ec.PhcId='".$data->phc."' AND ec.HscId='".$data->hsc."' AND av.status=1 ORDER BY av.picmeno ASC;");
...
$anarray[] = array( 
    ...
    "bloodSugartest" =>$row['bloodSugartest'],
