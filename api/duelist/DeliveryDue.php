...
$checkvaliduser = $db->prepare("SELECT mh.id,mh.picmeno,ec.motheraadhaarname,mh.edddate,ec.mothermobno,ec.PhcId,u.name FROM medicalhistory mh JOIN ecregister ec on ec.picmeNo=mh.picmeno JOIN users u on mh.createdBy=u.id WHERE YEAR(edddate) = YEAR(CURRENT_DATE()) AND MONTH(edddate) = MONTH(CURRENT_DATE()) AND ec.BlockId='".$data->block."' AND ec.PhcId='".$data->phc."' AND ec.HscId='".$data->hsc."' AND mh.status=1 ORDER BY mh.picmeno ASC;");
