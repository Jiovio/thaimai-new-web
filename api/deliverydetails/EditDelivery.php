...
if (!empty($data->picmeno)) {
    ...
    if ($mvalidchecknum ==1) {
        //print_r($mvalidchecknum);
        $inserid = $form->editdeliverydetails();
        // set response code - 201 created
        http_response_code(200);
        // tell the user
        echo json_encode(array("success" => "true", "error" => "false", "message" => "Delivery Details Updated Successfully."));
    } else {
       
        http_response_code(400);

        echo json_encode(array("success" => "false", "error" => "true","message" => "Please Check the fields.Delivery Details not Updated."));
...
// tell the Delivery Details data is incomplete
    ...
    echo json_encode(array("success" => "false", "error" => "true", "message" => "Unable to Update Delivery Details. "));
}
?>