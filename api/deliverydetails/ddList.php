...
$checkvaliduser = $db->prepare("SELECT * FROM deliverydetails dd JOIN ecregister ec ON ec.picmeNo=dd.picmeno WHERE ec.BlockId='".$data->block."' AND ec.PhcId='".$data->phc."' AND ec.HscId='".$data->hsc."' AND dd.status=1");
...
"injuction" =>$row['injuction']
    ...
    echo json_encode(
            array("success" => "true", "error"=> "false",  "DeliveryDetails" => $ddarray , "message" => "Delivery Details Registered List")
    );
    ...
    echo json_encode(
            array("success" => "false", "error"=>"true", "message" => "Delivery Details Data Not Found")
    );
}
?>
