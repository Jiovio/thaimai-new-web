...
$checkvaliduser = $db->prepare("SELECT * FROM postnatalvisit pv join ecregister ec on ec.picmeNo=pv.picmeNo WHERE ec.BlockId='".$data->block."' AND ec.PhcId='".$data->phc."' AND ec.HscId='".$data->hsc."' AND pv.status=1");
...
"bpDia" =>$row['bpDia'],
    ...
    echo json_encode(
            array("success" => "true", "error"=> "false",  "PostnatalList" => $PvArray , "message" => "Postnatal Visit Registered List")
    );
