...
document.getElementById("HscId").disabled = true;
}
...
function checkECdetails(Hsc, HscId, motheraadhaarid, husbandaadhaarid){
        $.ajax({
...
//	$("#BlockId").prop('disabled', false);	/* Controlling the result */
//	$("#PhcId").prop('disabled', false); /* Controlling the result */
    var ecfrno = $('#ecfrno').val();
    var ecfr   = $('#ecfr').val();
    GetECFRNo(ecfr, ecfrno);
	
}

function GetECFRNo(ecfr, ecfrno){
        $.ajax({
        url: "ajax/GetNextNo.php",
		async:false,
        type: "POST",
        data: {
            ecfr: ecfr, ecfrno: ecfrno
        },
		cache: false,
        success: function (result) {
           /* $('#suggesstion-box').html("");
