...
anRegDate='$rgdate', mrmbsEligible='$mrmbs',MotherAge='$MotherAge',HusbandAge='$HusbandAge', updatedat='$date',updatedBy='$userid' WHERE id=$id");
  if (!empty($uquery)) {
    echo "<script>alert('Updated Successfully');window.location.replace('http://localhost/thaimainew/forms/AnRegisterlist.php');</script>";
  }
 }
if (isset($_GET['del'])) {
  $id = $_GET['del'];
  ...
  mysqli_query($conn, "UPDATE anregistration SET status=0, deletedat='$date', deletedBy='$userid' WHERE status=1 AND id=$id");
    echo "<script>alert('Deleted Successfully');window.location.replace('http://localhost/thaimainew/forms/AnRegisterlist.php');</script>";
}
?>        
<!-- Content wrapper -->
