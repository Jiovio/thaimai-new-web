<?php 
$ErCntmq = mysqli_query($conn,"SELECT COUNT(id) AS ErCnt FROM ecregister WHERE BlockId='".$bloName."' AND PhcId='".$phcName."' AND status=1");
...
$AvCntmq = mysqli_query($conn,"SELECT COUNT(av.picmeno) AS AvCnt FROM antenatalvisit av JOIN ecregister ec ON av.picmeno=ec.picmeno WHERE ec.BlockId='".$bloName."' AND ec.PhcId='".$phcName."' AND av.status=1");
...
$HrCntmq = mysqli_query($conn,"SELECT COUNT(av.symptomsHighRisk) AS HrCnt FROM antenatalvisit av JOIN ecregister ec on av.picmeNo=ec.picmeno WHERE ec.BlockId='".$bloName."' AND ec.PhcId='".$phcName."' AND av.symptomsHighRisk NOT IN('NO','None','Nil') AND av.symptomsHighRisk!='' AND av.status=1");
...
$LmCntmq = mysqli_query($conn,"SELECT COUNT(id) AS LmCnt FROM hscmaster WHERE BlockId='".$bloName."' AND PhcId='".$phcName."'");
