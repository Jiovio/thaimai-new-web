<?php
$ErCntmq = mysqli_query($conn,"SELECT COUNT(id) AS ErCnt FROM ecregister WHERE BlockId='".$bloName."' AND PhcId='".$phcName."' AND HscId='".$hscName."' AND status=1");
...
$HrCntmq = mysqli_query($conn,"SELECT COUNT(av.symptomsHighRisk) AS HrCnt FROM antenatalvisit av JOIN ecregister ec on av.picmeNo=ec.picmeno WHERE ec.BlockId='".$bloName."' AND ec.PhcId='".$phcName."' AND ec.HscId='".$hscName."' AND av.symptomsHighRisk NOT IN('NO','None','Nil') AND av.symptomsHighRisk!='' AND av.status=1");
