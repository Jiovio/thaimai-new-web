  ...
  { */
	   ...
	   if(($usertype == 3) AND
		...
		/* if($usertype == 6) {
			...
			// print_r("user".$_SESSION['username']);
			/* if($rowh['username'] == 'customer_munibai')
			 {
				 print_r("Hi".$rowh['HscId']."*".$rowh['BlockId']."*".$rowh['PhcId']."*".$rowh['mobile']);
			 } 
			 
			 if($row['HscId']==525960 AND
			    $row['BlockId']==4370 AND
				$row['PhcId']==174624)
				{
					print_r("Mobile No".$row['mothermobno']);
				}	*/
			$match_fnd = "N";   
		  if(($usertype == 6) AND
