...
/*Getting Last Dose No */
 $im_picme = "";
						  $im_picme = $_GET['picmeNo'];
						  $immuneSql = mysqli_query($conn, "SELECT * FROM immunization WHERE picmeNo = '$im_picme' order by id desc LIMIT 0,1");
                          $immuneData = mysqli_fetch_array($immuneSql);
						  $im_dose_no = "";
						  $im_dose_no = $immuneData['doseNo'];
						  $im_dose_no_next = 0;
						  
						  if($im_dose_no > 0)
						  {
						  $im_dose_no_next = $im_dose_no + 1;
						  }
						  
						  $im_picme = $_GET['picmeNo'];
						  $deliverySql = mysqli_query($conn, "SELECT * FROM deliverydetails WHERE picmeno = '$im_picme'");
                          $deliveryData = mysqli_fetch_array($deliverySql);
						  
						  if($im_dose_no_next == 2) 
						  {	  
						  $ImmuneDate = date('Y-m-d', strtotime($deliveryData['deliverydate']. ' + 75 days'));
						  }
						 
						  if($im_dose_no_next == 3) 
						  {	  
						  $ImmuneDate = date('Y-m-d', strtotime($deliveryData['deliverydate']. ' + 105 days'));
						  }	 

                          if($im_dose_no_next == 4) 
						  {	  
						  $ImmuneDate = date('Y-m-d', strtotime($deliveryData['deliverydate']. ' + 270 days'));
						  }	 		

                           if($im_dose_no_next == 5) 
						  {	  
						  ...
						  {	  
						//  print_r("Hi"); exit;
						   echo "<script>alert('Already 5 doses taken. No more doses applicable for this picme.');
						   window.location.replace('{$siteurl}/forms/ImmunizationDtl.php?History=$im_picme');</script>";
						  }
...
//   exit;
    if (!empty($query)) {
            echo "<script>alert('Inserted Successfully');window.location.replace('{$siteurl}/forms/ImmunizationDtl.php?History=$picmeno');</script>";
		//	window.location.replace('{$siteurl}/forms/Immunization.php');</script>";
    } } } ?>
          <!-- Content wrapper -->
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Immunization /</span> Add Immunization Details
              <a href="ImmunizationDtl.php?History=<?php echo $_GET['picmeNo']; ?>"><button type="submit" class="btn btn-primary" id="btnBack">
                  ...
                  <div class="card mb-4">
					...
					<div class="row">
                        <div class="mb-3 col-md-6">
                          ...
                          <div class="frmSearch">
						  <input hidden id="picmenoImmune" name="picmeNo" onclick="return addImmuneValidate()" class="form-control" value=<?php echo $_GET['picmeNo']; ?> />
                          <label class="lblViolet"><?php echo $_GET['picmeNo']; ?>
                              </label>
						...
						<div class="mb-3 col-md-6">
                          <label class="form-label">Dose No. <span class="mand">* </span></label>
                            <select required name="doseNo" id="doseNo" disabled class="form-select" onclick="return addImmuneValidate()">
                           <?php 
						   						
                            $query = "SELECT enumid,enumvalue,doseNo FROM enumdata WHERE type=42 AND enumid=$im_dose_no_next;";
					...
					<div class="row">
						<div class="mb-3 col-md-6">
                          <label class="form-label">Dose Name <span class="mand">* </span></label>
                            <select required name="doseName[]" id="doseName" multiple="multiple" class="form-select" onclick="return addImmuneValidate()">
                          <?php 
                            $query = "SELECT enumid,enumvalue FROM enumdata WHERE type=43 AND doseNo=$im_dose_no_next;";
                            $exequery = mysqli_query($conn, $query);
