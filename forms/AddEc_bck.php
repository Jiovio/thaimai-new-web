        ...
        class="<?php if(!empty($type)) { echo $type . " display-block"; } else { echo $type . " display-none"; } ?>"><?php if(!empty($emessage)) { echo $emessage; } ?></div>
						...
						<div class="row">
						 <div class="col-6 mb-3">
                          ...
                          <select name="HscId" id="HscId" class="form-select" onclick="return addECValidate()" onchange="return ECFRNo" required>
                          <option value="">Choose...</option>
                           ...
                           // $query = "SELECT DISTINCT(HscId),HscName FROM hscmaster WHERE HscId='".$_SESSION["HscId"]."' ORDER BY HscId";
							$query = "SELECT DISTINCT(HscId),HscName FROM hscmaster WHERE BlockId='".$_SESSION["BlockId"]."' ORDER BY HscName";
                           } else if(($usertype == 3) ) {
                           // $query = "SELECT DISTINCT(HscId),HscName FROM hscmaster ORDER BY HscId";
						    $query = "SELECT DISTINCT(HscId),HscName FROM hscmaster WHERE BlockId='".$_SESSION["BlockId"]."' AND PhcId='".$_SESSION["PhcId"]."' ORDER BY HscName";
                           } else if(($usertype == 6)) {
							$query = "SELECT DISTINCT(HscId),HscName FROM hscmaster WHERE BlockId='".$_SESSION["BlockId"]."' 
							          AND PhcId='".$_SESSION["PhcId"]."' AND HscId='".$_SESSION["HscId"]."' ORDER BY HscName";   
						   }
						   else
						   {
							$query = "SELECT DISTINCT(HscId),HscName FROM hscmaster ORDER BY HscId";   
