...
$motstatus = mysqli_query($conn, "UPDATE ecregister SET status=3 WHERE picmeNo=".$picmeNo);
  ...
  if (!empty($query)) { ?> 
      <div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><h4><i class="icon fa fa-check"></i>Inserted Successfully</h4></div>
    <?php  header('location: PostnatalVisit.php');
   } else { ?>
                      ...
                      <form id="formAccountSettings" autocomplete="off" method="POST">
                        <div class="row">
                          <div class="mb-3 col-md-6">
                            ...
                            <?php   
                            $query = "SELECT picmeno FROM antenatalvisit";
