          ...
          <div class="content-wrapper">
			...
			<!-- Hoverable Table rows -->
              <div class="card"><h5 class="card-header">
                  <span class="text-muted fw-light">Reports / Postnatal Visit / </span> ECs Following Permanent Methods
               </h5>  
...
<!------------------------------------------------------------------------------------- Page Details + search button + Table -------------------------------------------------------->		   
    ...
    <?php
      ...
      $listQry = "SELECT pv.picmeNo,pv.id, pv.ppcMethod, ec.HscId, ar.anRegDate, ar.obstetricCode, ar.livingChildren, ec.VillageId, ec.PanchayatId, ar.MotherAge, ec.motheraadhaarname,pv.createdBy, ec.BlockId,ec.PhcId, ec.husbandaadhaarname, ec.mothermobno FROM postnatalvisit pv JOIN ecregister ec on ec.picmeNo=pv.picmeNo JOIN anregistration ar on ar.picmeno=pv.picmeNo
...
<!-------------------------------------------------------------- Download button + Submitting values to next page ------------------------------------------------------------------>
    <form action="ECPrmntListExp.php" method="post" id="filterform" style="width:100%";>	
