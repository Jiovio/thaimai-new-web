...
$HrCntmq = mysqli_query($conn,"SELECT count(ar.id) as HrCnt from anregistration ar JOIN ecregister ec on ar.picmeno=ec.picmeNo WHERE ar.status=1 AND NOT EXISTS (SELECT dd.picmeno FROM deliverydetails dd WHERE dd.picmeno = ar.picmeno)");
