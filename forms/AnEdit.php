...
if (!empty($data->motheraadhaarid)) {
    ...
    if ($mvalidchecknum >0) {
        $inserid = $form->editanregistration();
        ...
        echo json_encode(array("success" => "true", "error" => "false", "message" => "Antenatal Register Updated Successfully."));
    } else {
       
        http_response_code(400);

        echo json_encode(array("success" => "false", "error" => "true","message" => "Mother's Aadhaar ID Already Entered."));        
    }
}
} else {
    http_response_code(400);

    echo json_encode(array("success" => "false", "error" => "true","message" => "Please Check Mother's Aadhaar ID. Antenatal Register not Updated."));  
}
}
// tell the Antenatal Register data is incomplete
else {
    // set response code - 400 bad request

    http_response_code(400);

    echo json_encode(array("success" => "false", "error" => "true","message" => "Please Check the fields.Antenatal Register not Updated."));
