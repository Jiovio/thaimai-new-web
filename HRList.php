...
<!------------------------------------------------------------------------------------- Page Details + search button + Table -------------------------------------------------------->		   
    ...
    <?php
        ...
        $listQry = "SELECT ar.hrPregnancy,ar.gravida,ar.para,ar.livingChildren,ar.abortion,ar.childDeath,ar.bpSys,ar.bpDia,ar.motherWeight, ar.motherHeight, ar.updatedat, ar.createdat,ar.picmeno,ar.residentType, ec.motherdob, ar.id, ec.HscId, ec.VillageId, ec.PanchayatId, ar.anRegDate, ar.obstetricCode, ar.MotherAge, ec.motheraadhaarname,ar.createdBy, ec.BlockId,ec.PhcId, ec.husbandaadhaarname, ec.mothermobno FROM anregistration ar JOIN ecregister ec on ec.picmeNo=ar.picmeno
					...
					$row['lmpdate'] = $row_mh['lmpdate'];
						...
						print_r($row_mh['momVdrlRprResult']); print_r("After"); } */
                          ...
                          if($row_mh['hushivtestresult'] == "1") 
							...
							$row['symptomsHighRisk'] = "Moms VDRL Positive";	
							}
							else		
														 if($row_mh['husVdrlRprResult'] == "1") 
							{
							
							$row['symptomsHighRisk'] = "Husbands VDRL Positive";	
							}
														 else
				     ...
				     if($row['HscId']==$rowh['HscId'] AND
						...
						$row['PanchayatId']==$rowh['PanchayatId'])
						{
							//(ar.gravida > 2 OR ar.para > 2 OR ar.livingChildren > 2 OR ar.abortion > 2 OR ar.childDeath > 2 OR ar.bpSys >= 140 OR ar.bpDia >= 90 OR ar.motherWeight <= 40)"; 
										  ...
										  //  if($row['picmeno']=="133010118352"){
												
					//	print_r("Here!".$High_Risk_Ind.$AV_Rec_fnd);exit;}
